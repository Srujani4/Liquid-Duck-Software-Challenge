...
customer_data = pd.DataFrame({
    ...
    "type": [fake.random_element(elements=("Restaurant", "Bar", "GroceryStore")) for _ in range(5)],
...
sales_data = pd.DataFrame({
    ...
    "invoice_date": [fake.date_this_year().strftime('%Y-%m-%d') for _ in range(10)],
    "quantity": [fake.random_int(min=1, max=50) for _ in range(10)],
    #"net_price": [fake.random_float(min=10.0, max=100.0, ndigits=2) for _ in range(10)]
    "net_price": [round(random.uniform(10.0, 100.0), 2) for _ in range(10)]
})
...
for table in tables:
    ...
    result = conn.execute(f"SELECT * FROM {table};").fetchdf()  # Fetch as DataFrame
    print(result)
...
conn.execute("""
    ...
    GROUP BY GROUPING SETS (
        (p.supplier, p.brand, p.family, STRFTIME(CAST(s.invoice_date AS DATE), '%Y-%m')),
...
# Pivot: Create a view where columns are invoice months
conn.execute("""
    ...
    PIVOT (
        SUM(quantity) FOR invoice_date_month IN ('2024-01', '2024-02', '2024-03', '2024-04')
    );
""")
...
conn.execute("""
    ...
    SELECT supplier, brand, family, month_column AS invoice_date_month, quantity
    FROM pivoted_sales
    UNPIVOT (
        quantity FOR month_column IN ('2024-01', '2024-02', '2024-03', '2024-04')
    );
""")
